...
local function Toggle()
    ...
    enabled = not enabled
    if enabled then
    if enabledw then enabledw = not enabledw end
...
local function Togglew()
    ...
    enabledw = not enabledw
    if enabledw then
    if enabled then enabled = not enabled end
        for name, code in ipairs(KeyCode) do
        end
        for i = 1, #KeyCode do
            table.insert(_mems, menu.register_hotkey(KeyCode[i], Trigger))
 ...
 menu.register_hotkey(151, function() Trigger() end)
menu.add_toggle("Permanent 5 Stars", function() return enabled end, Toggle)
menu.add_toggle("Auto remove wanted", function() return enabledw end, Togglew)